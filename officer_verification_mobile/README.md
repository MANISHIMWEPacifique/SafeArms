...
4. In Connection Setup, first use EDIT API URL to apply the reachable backend address.
5. Use ENROLL DEVICE with a commander-generated 6-digit PIN only for first-time enrollment on that phone.
6. Build-time officer credentials remain supported for pre-provisioned builds.
...
3. Enter API Base URL (LAN/public URL reachable from that phone), then tap EDIT API URL.
4. If this phone is not yet enrolled, enter the 6-digit Enrollment PIN and tap ENROLL DEVICE.
5. Return to the incoming request screen and continue custody verification.

If this phone is already enrolled and only the server address changed, do not re-enroll. Use EDIT API URL only.

For first-time enrollment, the app exchanges the PIN for officer and device credentials and stores them locally for runtime use.
...
- Use EDIT API URL in Connection Setup whenever the backend IP/host changes.
- Use ENROLL DEVICE only for first-time phone enrollment with a valid 6-digit PIN.
- If a phone is already enrolled, re-enrollment remains blocked until active enrollment is removed on the web dashboard.
